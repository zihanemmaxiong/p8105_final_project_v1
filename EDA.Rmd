...
**High–High:** ZIP codes with high values surrounded by neighbors that also have high values. These are “hotspots” of spatial clustering.

**Low–Low:** ZIP codes with low values surrounded by low-value neighbors. These are “coldspots.”
...
ZIP codes classified as “Not significant” do not show statistically significant clustering at the chosen α level.
```{r lisa-weights}
...
The bivariate map highlights a core set of neighborhoods where interventions could have the biggest impact: areas with high values for both indicators. At the same time, the presence of “mismatched” ZIPs (rat-high / restaurant-low and restaurant-high / rat-low) suggests that rodent control and food safety policies cannot focus solely on restaurants. Housing quality, waste infrastructure, and neighborhood connectivity also likely play important roles in shaping where rats thrive.
```{r}
# 1. 计算三分位 + 双变量编码
ny_zip_sf <- ny_zip_sf |>
  dplyr::mutate(
